    ...
    public class Excel
    {
            ...
            try
            {
                ...
                for (int i = 0; i < GroupByOrder.Count; i++)
                {
                    for (int j = 0; j < GroupByOrder[i].Profile.Count; j++)
                    {
                        ...
                        WS.Cells[rowCntReport + 1, 5].Value = GroupByOrder[i].Profile[j].Sum(p => p.Weight*p.Count);
                    }
                }
                ...
                WS.Cells[last + 1, 5].Value = details.Sum(p => p.Weight * p.Count);
        ...
        public bool ReportMarkSelected(List<Order> Report, String FileName)
        {
            ...
            try
            {
                ...
                List<Order> sortOrders = Report.OrderBy(p => p.Number).ThenBy(p => p.Mark).ThenBy(p => p.List).ToList();
                ...
                for (int i = 0; i < sortOrders.Count; i++)
                {
                    ...
                    for (int g = 0; g < GroupByOrder.Count; g++)
                    {
                        for (int j = 0; j < GroupByOrder[g].Profile.Count; j++)
                        {
                            ...
                            WS.Cells[rowCntReport + 1, 4].Value = sortOrders[i].List;
                            WS.Cells[rowCntReport + 1, 5].Value = GroupByOrder[g].Mark;
                            WS.Cells[rowCntReport + 1, 6].Value = GroupByOrder[g].Profile[j].Key;
                            if (!String.IsNullOrEmpty(GroupByOrder[g].Profile[j].Select(p => p.GostName).FirstOrDefault()))
                            {
                                WS.Cells[rowCntReport + 1, 7].Value = GroupByOrder[g].Profile[j].Select(p => p.GostName).First();
                            }
                            else
                            {
                                WS.Cells[rowCntReport + 1, 7].Value = "Гост не найден";
                            }
                            WS.Cells[rowCntReport + 1, 8].Value = GroupByOrder[g].Profile[j].Sum(p => p.Weight * p.Count);
                        }
                    }
                }
                ...
                WS.Cells["B2:H" + last].Style.Border.Top.Style = ExcelBorderStyle.Thin;
                WS.Cells["B2:H" + last].Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
                WS.Cells["B2:H" + last].Style.Border.Left.Style = ExcelBorderStyle.Thin;
                WS.Cells["B2:H" + last].Style.Border.Right.Style = ExcelBorderStyle.Thin;

                WS.Cells["B2:H" + last].Style.Font.Name = "Times New Roman";
                WS.Cells["B2:H" + last].Style.Font.Size = 14;
                WS.Cells["B2:H" + last].AutoFitColumns();
                WS.Cells["B2:H" + last].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
